...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        # ADD CODE HERE
        angleTurn = random.gauss(motion[0], self.steering_noise) # alpha with noise (steering wheel slip)

        distance = random.gauss(motion[1], self.distance_noise)  # distance d with noise
        lengthOfCar = self.length
        carAngle = random.gauss(self.orientation, self.bearing_noise) # theta with noise (wheel slip)
        
        turningAngle = (distance / lengthOfCar) * tan(angleTurn) # beta
        
        if (fabs(turningAngle) < 0.001 ):
            # Car goes straight
            xNew = self.x  + distance * cos(carAngle)
            yNew = self.y + distance * sin(carAngle)
            carAngleNew = (carAngle + turningAngle ) % (2*pi)
            result = robot()
            ...
            result.set(xNew, yNew, carAngleNew)
        else:
            # Car turns
            radiusC = distance / float(turningAngle) # R
            cx = self.x - (sin(carAngle) *radiusC)
            cy = self.y + (cos(carAngle) * radiusC)
            
            xNew = cx + (sin(carAngle + turningAngle) * radiusC)
            yNew = cy - (cos(carAngle + turningAngle) * radiusC)
            carAngleNew = (carAngle + turningAngle ) % (2*pi)
            result = robot()            
            result.distance_noise = self.distance_noise
            result.steering_noise = self.steering_noise
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            ...
            heading = heading - self.orientation
            if add_noise:
                heading += random.gauss(0.0, self.bearing_noise)
            Z.append(heading)
